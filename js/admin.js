...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Automatic real-time polling with window focus triggers
  setInterval(() => {
  ...
  // Trigger immediate cloud fetch when window gets focus or user switches back to tab
  window.addEventListener('focus', () => {
    fetchCloudDataToLocal();
    if (localStorage.getItem('conesess_admin_auth') === 'true') renderAdminAll();
  });
});
...
const CLOUD_SYNC_ENDPOINTS = [
  "https://crudcrud.com/api/8484295837b1490394750b39c131212e/submissions",
...
function renewCloudSyncEndpointKey() {
  const customKey = prompt("Entrez votre clé CrudCrud à 32 caractères ou votre URL d'API personnalisée :", "8484295837b1490394750b39c131212e");
  if (customKey && customKey.trim()) {
    let cleanUrl = customKey.trim();
    if (!cleanUrl.startsWith('http')) {
      cleanUrl = `https://crudcrud.com/api/${cleanUrl}/submissions`;
    }
    if (!CLOUD_SYNC_ENDPOINTS.includes(cleanUrl)) {
      CLOUD_SYNC_ENDPOINTS.unshift(cleanUrl);
    }
    localStorage.setItem('conesess_custom_cloud_endpoint', cleanUrl);
    const label = document.getElementById('label-active-cloud-endpoint');
    if (label) label.textContent = cleanUrl;
    forceRealtimeSyncRefresh();
    showToast("Point d'accès Cloud mis à jour et réactivé !");
  }
}
...
function switchAdminTab(tabId) {
  ...
  // Automatically close mobile sidebar menu drawer on selection
  const sidebar = document.querySelector('.admin-sidebar');
  const overlay = document.getElementById('admin-sidebar-overlay');
...
function renderAdminAll() {
  ...
  // Metrics
  const totalSubmissionsCount = webForms.length;
  const approvedMembersCount = members.filter(m => m && m.status === 'Approuvé').length;
  const pendingMembersCount = webForms.filter(f => f && f.status === 'En attente').length;
  const steeringCandidates = webForms.filter(f => f && f.type === 'Candidature Comité de Pilotage');
  const steeringCount = steeringCandidates.length;
  const uniqueRegionsCount = new Set(webForms.map(w => w && w.region).concat(members.map(m => m && m.region)).filter(Boolean)).size;
  const generatedBadgesCount = members.filter(m => m && m.badgeStatus && m.badgeStatus !== 'Non généré').length;
  const pendingAdminsCount = adminUsers.filter(u => u && u.status === 'En attente').length;
  ...
  if (statTotal) statTotal.textContent = totalSubmissionsCount;
  if (statWebFormsTotal) statWebFormsTotal.textContent = totalSubmissionsCount;
  ...
  if (statRegionsCovered) statRegionsCovered.textContent = `${uniqueRegionsCount} / 14`;
  ...
  if (countNavWebForms) countNavWebForms.textContent = totalSubmissionsCount;
